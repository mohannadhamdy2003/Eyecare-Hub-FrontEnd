    ...
    try {
      ...
      if (existingUser.data.length > 0) {
        return thunkAPI.rejectWithValue(
          "Invalid credentials!, email is already exists"
        );
      }
  ...
  async ({ email, password }, thunkAPI) => {
    try {
      ...
      if (res.data.length === 0) {
        return thunkAPI.rejectWithValue(
          "Invalid credentials!, user is not exists"
        );
      } else if (res.data[0]?.bannInfo?.isBanned) {
        return thunkAPI.rejectWithValue(
          `You was banned because ${res.data[0].bannInfo.reason}`
        );
      }
  ...
  async ({ operation, data }, thunkAPI) => {
    try {
      ...
      if (operation === "add") {
        ...
        const productIndex = cartInfo.cart.findIndex(
          (p) => p.product.id === data
        );
...
export const AppointmentOperationsApi = createAsyncThunk(
  "auth/AppointmentOperations",
  async ({ operation, data }, thunkAPI) => {
    ...
    try {
      ...
      //
        ...
        return thunkAPI.rejectWithValue(
          "Appointment not found in user's appointments"
        );
      }
    } catch (error) {
